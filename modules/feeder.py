...
# cmdLineOptions['cbmc'] = '  --unwind 1 --no-unwinding-assertions --32 ' # For the competition ... this is modified for our purposes (need to choose custom loop unwinding length)
...
class feeder(core.module.BasicModule):
    ...
    def loadfromstring(self, string, env):
        ...
        # custom options for cbmc backend
        self.__ps_rounds = int(env.paramvalues['ee'])
        cmdLineOptions['cbmc'] = ' p --unwind %s ' % (str(self.__ps_rounds))
        ...
        if backend == 'smack':
            ...
            # block log output to stderr/stdout
