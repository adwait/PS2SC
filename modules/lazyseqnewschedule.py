...
class lazyseqnewschedule(core.module.Translator):
    ...
    # keep count of number of copies of a thread
    # dict with str:threadName -> int 
    __threadCopies = {}
    ...
    __can_promise = True
    __enable_prom = ''
    # underapproximation ... certification depth
    __certDepth = 0
    ...
    def init(self):
        ...
        self.addOutputParam('totalThreads')
        self.addOutputParam('threadIndex')


    def _is_ps_helper(self, n):
        return 'lazyps' in n

    # multi dimensional arrays not supported
    def get_node_suffix(self, n):
        suffix = ""
        while not isinstance(n, pycparser.c_ast.ID):
            if isinstance(n.subscript, pycparser.c_ast.ID):
                suffix = '[' + n.subscript.name + ']'
            else:
                suffix = '[' + n.subscript.value + ']'
            n = n.name
        return suffix

    # supports multidimensional arrays
    # get the indexing suffix of arrays
    def get_gennode_suffix(self, n):
        suffix = ''
        while not isinstance(n, pycparser.c_ast.ID):
            suffix += '[' + n.subscript.value + ']'
            n = n.name
    ...
    # get identifier of array (sans indexing)
    ...
    # multi dimensional arrays not supported
    def get_index_var(self, var_name, suffix):
        nVars = 0
        for k in self.Parser.varNames['']:
            if(k == var_name):
                nVars += 1
                break
            a = sum(self.Parser.varSize[('', k)])
            if a == 0:
                a = 1
            nVars += a
        if suffix != '':
            index = suffix.strip('[').strip(']')
            if index.isdigit():
                # nVars = nVars + int(suffix.strip('[').strip(']'))
                nVars = nVars + int(index)
                return str(nVars)
            else:
                return str(nVars) + '+%s' % index
        # print(self.Parser.varSize[('', var_name)], var_name)
        return str(nVars)
    ...
    def get_indices_position(self, l, indices):
        a = 0
        templ = l
        tempind = indices
        while templ:
            a = a*templ[0] + tempind[0]
            templ = templ[1:]
            tempind = tempind[1:]
        return a

    # general variable to index conversion
    def get_genindex_var(self, var_name, suffix):
        nVars = 0
        indices = list(map(lambda x: int(x) if x else 0, suffix.strip('[').strip(']').split('][')))
        ...
        for k in self.Parser.varNames['']:
            ...
            if(k == var_name):
                ...
                break
            # a = sum(self.Parser.varSize[('', k)])
            a = self.list_prod(self.Parser.varSize[('', k)])
            # if a == 0:
            #     a = 1
            nVars += a
        # if suffix != '':
            # nVars = nVars + int(suffix.strip('[').strip(']'))
            # print(self.Parser.varSize[('', var_name)], var_name)
        nVars += self.get_indices_position(self.Parser.varSize[('', var_name)], indices)
        return str(nVars)
    ...
    def get_local_view_name(self):
        ...
        s = '_'.join(self.__currentThread.split('_')[:1])
        return "__cs_local_" + s +"__view"

    def getLocalVarPrefix(self):
        if 'pscopy' in self.__currentFunction:
            return '__cs_local_' + self.__currentFunction + '_'
        return "__cs_local_" + '_'.join(self.__currentThread.split('_')[:1]) + "_"

    ''' 
    def get_publish_message_stats(self, var_arr, flag):
    ...
    ''' 
    def get_update_view_stats(self, var_arr, threadname):
        # only 1D arrays supported
        var_num = self.get_index_var(var_arr[0], var_arr[1])
        message_num = self.getLocalVarPrefix() + '_ps_non_det_int_var'
        # pre - inlined function since variable-name of choice is not known statically (and variable name is embedded)
        s = ''
        p = self.__time_bits
        s += '\n'
        # s += '  __CPROVERs_assume((%s_view_l)%s[%s]);\n' % (var_arr[0], var_arr[1], threadname)
        s += '  __CPROVER_assume(((%s_view_t)%s[%s] < %s_ps_time%s[%s]) || (((%s_view_t)%s[%s] < %s_ps_time%s[%s]) && (%s_view_l)%s[%s]));\n' % (var_arr[0], var_arr[1], threadname, var_arr[0], var_arr[1], message_num, var_arr[0], var_arr[1], threadname, var_arr[0], var_arr[1], message_num, var_arr[0], var_arr[1], threadname)
        s += '  __CPROVER_assume(_ps_var[%s] == %s);\n' % (message_num, var_num)
        s += '  %s_view%s[%s] = %s_values%s[%s];\n' % (var_arr[0], var_arr[1], threadname, var_arr[0], var_arr[1], message_num)
        s += '  (%s_view_t)%s[%s] = %s_ps_time%s[%s];\n' % (var_arr[0], var_arr[1], threadname, message_num)
        s += '  (%s_view_v)%s[%s] = 1;\n' % (var_arr[0], var_arr[1], threadname)    
        if self.__can_promise: s += '  %s_extView%s[0] = 1;\n' % (var_arr[0], var_arr[1], threadname)
        return s 
    '''


    '''    # not actually useful to define as separate function ... may decide to replace with update_view later
    def get_global_read_stats(self, var_name, view_name):
        s = ''
        s += '__CPROVER_assume(%s < %s);\n' % (self._helper_names("numEE"), self._helper_names("rounds"))
        if 'pscopy' in self.__currentFunction:
            parentthread = '__cs_param_%s__ps_t1' % (self.__currentFunction)
            s += '__CPROVER_assume(%s_liveChain%s[0] == 0);\n' % (var_name[0], var_name[1], parentthread)
        # else:
        #   s += '__CPROVER_assume(%s_liveChain%s[ps_thread_num] == 0);\n' % (var_name[0], var_name[1])
        s += self.getLocalVarPrefix() + "_ps_non_det_int_var = nondet_uint();\n"
        s += "__CPROVER_assume(" + self.getLocalVarPrefix() + "_ps_non_det_int_var < %s);\n" % self._helper_names("index_used")
        if 'pscopy' in self.__currentFunction:
            parentthread = '__cs_param_%s__ps_t1' % (self.__currentFunction)
            s += self.get_update_view_stats(var_name, parentthread)
        else:
            s += self.get_update_view_stats(var_name, 'ps_thread_num')
        if 'pscopy' not in self.__currentFunction:
            s += "%s = %s + 1;\n" % (self._helper_names("numEE"), self._helper_names("numEE"))
        return s
    '''

    
    def get_update_var(self, name):
        return "_%s_update_helper_var" % self.Parser.varTypeUnExpanded[('', name)]

    def get_ps_inline_prom_free(self, node):
        s = ''
        view_name = self.get_local_view_name()
        parentthread = 'ps_thread_num'
        reserved_func = node.name.name

        def get_rval_base_string(rval_node):
            if type(rval_node) == pycparser.c_ast.Constant:
                return rval_node.value
            elif type(rval_node) == pycparser.c_ast.ID:
                return self.get_node_name(rval_node) + self.get_node_suffix(rval_node)
            else:
                print("Error: not base node in get_rval_base_string")
                exit(0)
        ...
        if 'rlx' in reserved_func or 'ra' in reserved_func or 'cas' in reserved_func:
            ...
            update_helper = self.get_update_var(var_name[0])
            index_var = self.get_index_var(var_name[0], var_name[1])
            if type(node.args.exprs[1]) == pycparser.c_ast.BinaryOp:
                bin_node = node.args.exprs[1]
                register_var = '%s %s %s' % (get_rval_base_string(bin_node.left), bin_node.op, get_rval_base_string(bin_node.left))
            else:
                register_var = get_rval_base_string(node.args.exprs[1])
            
            if 'cas' in reserved_func:
                if type(node.args.exprs[2]) == pycparser.c_ast.BinaryOp:
                    bin_node = node.args.exprs[2]
                    register_var2 = '%s %s %s' % (get_rval_base_string(bin_node.left), bin_node.op, get_rval_base_string(bin_node.left))
                else:
                    register_var2 = get_rval_base_string(node.args.exprs[2])
            # if type(node.args.exprs[1]) == pycparser.c_ast.Constant:
            #     register_var = node.args.exprs[1].value
            # elif type(node.args.exprs[1]) == pycparser.c_ast.BinaryOp:
            #     bin_node = node.args.exprs[1]
            #     register_var = '%s %s %s' % (bin_node.left, bin_node.op, bin_node.right)
            # elif type(node.args.exprs[1]) == pycparser.c_ast.ID:
            #     register_var = self.get_node_name(node.args.exprs[1]) + self.get_node_suffix(node.args.exprs[1])
            
        if reserved_func == self.reserved_functions['read_rlx']:
            s += sts.get_read(self.__currentFunction, var_name, index_var, register_var, 'rlx', False)
        elif reserved_func == self.reserved_functions['read_ra']:
            s += sts.get_read(self.__currentFunction, var_name, index_var, register_var, 'ra', False)
        elif reserved_func == self.reserved_functions['write_rlx']:
            s += sts.get_write_psfree(self.__currentFunction, var_name, index_var, register_var, 'rlx')
        elif reserved_func == self.reserved_functions['write_ra']:
            s += sts.get_write_psfree(self.__currentFunction, var_name, index_var, register_var, 'ra')
        elif reserved_func == self.reserved_functions['cas']:
            s += sts.get_cas_psfree(self.__currentFunction, var_name, index_var, register_var, register_var2, 'rlx')
        elif reserved_func == self.reserved_functions['fence']:
            s += 'scfence_ps(%s);\n' % parentthread

        return s

    def get_ps_inline(self, node):
        s = ''
        view_name = self.get_local_view_name()
        parentthread = 'ps_thread_num'
        reserved_func = node.name.name
        ...
        if reserved_func == self.reserved_functions['read_rlx']:
            s += sts.get_read(self.__currentFunction, var_name, index_var, register_var, 'rlx', True)
        elif reserved_func == self.reserved_functions['read_ra']:
            s += sts.get_read(self.__currentFunction, var_name, index_var, register_var, 'ra', True)
        elif reserved_func == self.reserved_functions['write_rlx']:
            s += sts.get_write_psfull(self.__currentFunction, var_name, index_var, register_var, 'rlx')
        elif reserved_func == self.reserved_functions['write_ra']:
            s += sts.get_write_psfull(self.__currentFunction, var_name, index_var, register_var, 'ra')
        elif reserved_func == self.reserved_functions['cas']:
            s += sts.get_cas_psfull(self.__currentFunction, var_name, index_var, register_var, register_var2, 'rlx')
        elif node.name.name == self.reserved_functions['save']:
            s += 'save_state_ps(ps_thread_num);\n'
        elif node.name.name == self.reserved_functions['load']:
            s += 'load_state_ps(ps_thread_num);\n'
        elif reserved_func == self.reserved_functions['conscheck']:
            if '_pscopy' in self.__currentFunction:
                print('error: conscheck should be called only by parent')
                exit(0)
            else:
                ...
                for k in self.Parser.varNames['']:
                    ...
                    for indices in self.gen_all(self.Parser.varSize[('', k)]):
                        s += '  __CPROVER_assume(%s_liveChain%s[0] == 0);\n' % (k, indices)
                        s += '  __CPROVER_assume(%s_liveChain%s[0] == 0);\n' % (k, indices)
                        var_name = [k, indices, k + indices]
                        for messit in range(self.__ps_rounds + 1):
                            s += 'if(%s_%s[%s] == 0 && %s_ps_reserve%s[%s] == ps_thread_num){;\n' % (self._helper_names("usedReservation"), var_name[2], messit, var_name[0], var_name[1], messit)
                            s += '  %s_ps_reserve%s[%s] = 0;\n' % (var_name[0], var_name[1], messit)
                            s += '  %s_%s[%s] = 1;\n' % (self._helper_names('upd'), var_name[2], messit)
                            s += '}\n'
                            s += '%s_%s[%s] = 0;\n' % (self._helper_names("usedReservation"), var_name[2], messit)
        elif reserved_func == self.reserved_functions['fence']:
            s += 'scfence_ps(%s);\n' % parentthread
    ...
    def get_ps_inline(self, node):
        ...
        if 'pscopy' in self.__currentFunction:
            ...
            if node.name.name == self.reserved_functions[0] or node.name.name == self.reserved_functions[1] or node.name.name == self.reserved_functions[2]:
                ...
                if node.name.name == self.reserved_functions[0]:
                    s += 'if(nondet_bool()){\n'
                    ...
                    s += '  %s_view_l%s[%s] = 1;\n' % (var_name[0], var_name[1], parentthread)
                    s += '  if(nondet_bool()){\n'
                    s += '    if(%s_liveChain%s[0]){\n' % (var_name[0], var_name[1], parentthread)
                    s += '      %s = %s_view_t%s[%s] + 1;\n' % (localvar, var_name[0], var_name[1], parentthread)
                    s += '    }\n'
                    s += '    else{\n'
                    s += '      %s = nondet_uint();\n' % (localvar)
                    s += '    }\n'
                    s += '    __CPROVER_assume(%s <= 2*%s);\n' % (localvar, self._helper_names('rounds'))
                    s += '    __CPROVER_assume(%s > %s_view_t%s[%s]);\n' % (localvar, var_name[0], var_name[1], parentthread)
                    s += '    %s = %s;\n' % (timevar, localvar)
                    ...
                    s += '    if(nondet_bool()){\n'
                    s += '          __CPROVER_assume(%s < %s);\n' % (self._helper_names('numEE'), self._helper_names('rounds'))
                    s += '          %s' % (self.get_publish_message_stats(var_name, '255'))
                    s += '          %s_liveChain%s[0] = 0;\n' % (var_name[0], var_name[1], parentthread)
                    s += '          %s_%s[%s] = 1;\n' % (self._helper_names('save_unavail'), var_name[2], localvar)
                    s += '          %s_%s[%s] = 1;\n' % (self._helper_names('save_upd'), var_name[2], localvar)
                    s += '          %s = %s + 1;\n' % (self._helper_names('numEE'), self._helper_names('numEE'))
                    s += '    }\n'
                    if not self.__weak_cons:
                        s += '    else{\n'
                        s += '          %s_liveChain%s[0] = 1;\n' % (var_name[0], var_name[1], parentthread)
                        s += '    }\n'
                    s += '  }\n'
                    s += '  else{\n'
                    s += '      %s = nondet_uint();\n' % (localvar)
                    s += '      __CPROVER_assume(%s < %s);\n' % (localvar, self._helper_names('index_used'))
                    s += '      __CPROVER_assume(_ps_time[%s] > %s);\n' % (localvar, timevar)
                    s += '      __CPROVER_assume(_ps_flag[%s] == %s);\n' % (localvar, parentthread)
                        # only 1D arrays supported
                    s += '      __CPROVER_assume(_ps_var[%s] == %s);\n' % (localvar, self.get_index_var(var_name[0], var_name[1]))
                    s += '      __CPROVER_assume(%s_values%s[%s] == %s);\n' % (var_name[0], var_name[1], localvar, valvar)
                    s += '          _ps_flag[%s] = 255;\n' % (localvar)
                    s += '          %s_liveChain%s[0] = 0;\n' % (var_name[0], var_name[1], parentthread)
                    s += '          %s = _ps_time[%s];\n' % (timevar, localvar)
                    s += '  }\n'
                    ...
                    s += '}\n'
                    s += '  %s_extView%s[0] = 0;\n' % (var_name[0], var_name[1], parentthread)
                    s += '  %s_view_u%s[%s] = %s;\n' % (var_name[0], var_name[1], parentthread, lvar)
                    s += '  %s_view_v%s[%s] = 1;\n' % (var_name[0], var_name[1], parentthread)
                    ...
                    s += 'if(nondet_bool()){\n'
                    s += '  __CPROVER_assume(!%s_extView%s[0]);\n' % (var_name[0], var_name[1], parentthread)
                    s += '  %s_liveChain%s[0] = 1;\n' % (var_name[0], var_name[1], parentthread)
                    s += '  %s =  0;\n' % (lvar)
                    s += '}\n'
                    s += 'else{\n'
                    s += '  if(nondet_bool()){\n'
                    s += '      if(%s_view_u%s[%s] || %s_liveChain%s[0]){\n' % (var_name[0], var_name[1], parentthread, var_name[0], var_name[1], parentthread)
                    s += '          %s = %s + 1;\n' % (localvar, timevar)
                    s += '      }\n'
                    s += '      else{\n'
                    s += '          %s = nondet_uint();\n' % (localvar)
                    ...
                    s += '%s_extView%s[0] = 0;\n' % (var_name[0], var_name[1], parentthread)
                    s += '  %s_view_v%s[%s] = 1;\n' % (var_name[0], var_name[1], parentthread)
                    # s += '__CPROVER_assume(%_view_l);\n'
            elif node.name.name == self.reserved_functions[4]:
                print 'error - conscheck() should not be called by helper'
                ...
                #       continue
                #   for indices in self.gen_all(self.Parser.varSize[('', k)]):
                #       s += '  __CPROVER_assume(!%s_liveChain%s[ps_thread_num]);\n' % (k, indices)
            elif node.name.name == self.reserved_functions[5]:
                # s += 'save_state_ps(ps_thread_num);\n'
                print 'error - save_state() should not be called by helper'
            elif node.name.name == self.reserved_functions[6]:
                print 'error - load_state() should not be called by helper'
            elif node.name.name == self.reserved_functions[7]:
                s += 'scfence_ps(%s);\n' % parentthread
                # s += 'load_state_ps(ps_thread_num);\n'
                # elif node.name.name == self.reserved_functions[5]:
                # to add arw operations translation
                # also add nondet_bool -> ps_nondet conversion
        else:
            parentthread = 'ps_thread_num'
            if node.name.name == self.reserved_functions[0] or node.name.name == self.reserved_functions[1] or node.name.name == self.reserved_functions[2]:
                name = self.get_node_name(node.args.exprs[0])
                ...
                s += 'load_state_ps(ps_thread_num);\n'
            elif node.name.name == self.reserved_functions['fence']:
                s += 'scfence_ps(%s);\n' % parentthread
                # elif node.name.name == self.reserved_functions[5]:
        return s
    '''

    ''' publish and update_view not required as function based translation not used '''
    # def get_publish_stats(self, var_name):
    #     pass

    # def get_update_view_stats(self, var_name):
    #     pass
    ...
    def loadfromstring(self, string, env):
        ...
        if 'cDep' in env.paramvalues:
            self.__certDepth = int(env.paramvalues['cDep'])
        ...
        self.setOutputParam('totalThreads', self.__threadCount)
    ...
    def visit_FuncDef(self, n):
        ...
        if (n.decl.name.startswith('__CSEQ_atomic_') or
            ...
            n.decl.name in self.Parser.funcReferenced  or # <--- functions called through pointers are not inlined yet
            # helper 'pscopy' function are replicated as is
            'pscopy' in self.__currentFunction):   
    ...
    def visit_Return(self, n):
        if self.__currentThread != '__CSEQ_assert' and self.__currentThread not in self.Parser.funcReferenced and not self.__atomic and 'pscopy' not in self.__currentThread:
    ...
    def visit_FuncCall(self, n):
        ...
        if fref == core.common.changeID['pthread_create']: # TODO re-write AST-based (see other modules)
            ...
            if fName not in self.__threadName:
                ...
                # threadVar to pass on mappings to future modules
                self.__threadVar[n.args.exprs[0].expr.name] = self.__threadCount
            else:
                # when visiting from the 2nd time on (if it happens),
                # reuse the old thread indexS!
                args = args + ', %s' % (self.__threadIndex[fName])

            fref = "merge_views_ps(0, %s);\n" % (self.__threadIndex[fName]) + fref
            self.__firstThreadCreate = True

        # merge views for main ASSERT check 
        if fref == core.common.changeID['pthread_join']:
            fName = args[:args.rfind(',')]
            fName = fName[fName.rfind(',')+2:]
            fName = '_' + fName.replace('&', '')
            if fName in self.__threadVar:
                return  fref + '(' + args + ')' + ";\n merge_views_ps(%s, 0)" % ( self.__threadVar[fName]) 
        ...
        # catch ps translation functions for translation
        if self.is_function_reserved(n) and n.name.name != self.reserved_functions['merge_view']:
            self.__globalMemoryAccessed = True
            if self.__can_promise and self.__enable_prom in self.__currentFunction: 
                return self.get_ps_inline(n)
            else: 
                return self.get_ps_inline_prom_free(n)        

        if n.name.name == '__CSEQ_assert' and 'pscopy' in self.__currentFunction:
            return ''
