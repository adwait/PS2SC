...
class Translator(BasicModule,pycparser.c_generator.CGenerator):
	...
	def __init__(self):
		...
		self.reserved_functions = {	'read_rlx': 'rlx_read_ps', \
									'write_rlx': 'rlx_write_ps', \
									'read_ra': 'ra_read_ps', \
									'write_ra': 'ra_write_ps', \
									'cas': 'cas_ps', \
									'merge_view': 'merge_main_view', \
									'conscheck': 'conscheck_ps', \
									'save': 'savestate_ps', \
									'load': 'loadstate_ps', \
									'fence': 'sc_fence'}	# function names used during first phase of translation
		self.struct_dict = {'view' : ' view_ps', 'message' : ' message_ps '}		# used for temporary struct names in translation


	def _struct_names(self, x):
		x = x.lower()
		return self.struct_dict[x]

	def _helper_names(self, x):	
		# creates temporaries for PS translation
		return "_lazyps_" + x

	def is_function_reserved(self, n):
		return n.name.name in self.reserved_functions.values()
